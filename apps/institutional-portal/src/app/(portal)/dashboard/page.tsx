...
export default function DashboardPage() {
  ...
  const completedOrders =
    orderList?.filter((o) => o.status === "COMPLETED").length ?? 0;
  const totalVolume =
    orderList
        ...
        {[
          {
            ...
            value: `${parseFloat(balance?.balance ?? "0").toFixed(4)} AUR`,
          ...
          {
            label: "Gold Price",
            value: balance?.goldPriceUsd
              ? formatUsd(balance.goldPriceUsd) + "/oz"
              : "—",
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link
              href="/orders?new=buy"
              className="flex items-center justify-center rounded-lg border border-gold-300 bg-gold-50 py-3 text-sm font-medium text-gold-700 hover:bg-gold-100 transition-colors"
            >
              Buy Tokens
            </Link>
            <Link
              href="/orders?new=sell"
              className="flex items-center justify-center rounded-lg border border-gray-300 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
            >
              Sell Tokens
            </Link>
            <Link
              href="/reports"
              className="col-span-2 flex items-center justify-center rounded-lg border border-gray-300 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
            >
              Download Report
            </Link>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <Link
            href="/orders"
            className="mt-4 block text-center text-xs text-gold-600 hover:underline"
          >
            View all orders →
          </Link>
        </div>
