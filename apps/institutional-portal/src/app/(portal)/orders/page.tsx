...
const orderSchema = z.object({
  ...
  amountGhs: z
    ...
    .refine(
      ...
      `Minimum GH₵${TENANT.minimumGhs.toLocaleString()}`,
    ),
...
function OrdersPageInner() {
  ...
  const setGhsFromOz = (oz: number) => {
    ...
    setValue("amountGhs", (oz * goldPriceUsd * ghsRate).toFixed(2), {
      shouldValidate: true,
    });
  };
  ...
  const createMutation = useMutation({
    mutationFn: (data: OrderForm) => {
      if (!ghsRate)
        throw new Error("Exchange rate unavailable. Please try again.");
                ...
                {inputMode === "oz" && type === "BUY" ? (
                  <div>
                    <input
                      ...
                      onChange={(e) =>
                        setGhsFromOz(parseFloat(e.target.value) || 0)
                      }
                    />
                    <p className="mt-1 text-xs text-gray-400">
                      {amountGhs && ghsRate
                        ? `= ${formatGhsDirect(amountGhs)}`
                        : "Enter oz to calculate"}
                    </p>
                  </div>
                ) : (
                  <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {inputMode === "oz"
                      ...
                      : USD_PRESETS.map((p) => (
                          ...
                          >
                            {ghsRate
                              ? `GH₵${Math.round(p * ghsRate).toLocaleString()}`
                              : `$${p.toLocaleString()}`}
                          </button>
                        ))}
                  </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              {goldPriceUsd > 0 && (
                <span className="text-xs text-gray-400 dark:text-gray-500">
                  ...
                  {ghsRate &&
                    ` · GH₵${(goldPriceUsd * ghsRate).toLocaleString("en-US", { maximumFractionDigits: 0 })}/oz`}
                </span>
              )}
            </div>
          </form>
                ...
                {orderList.map((order) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 font-medium text-gray-900 dark:text-gray-100">
                      ...
                      <p className="text-xs text-gray-400 font-normal mt-0">
                        {formatUsd(order.amountUsd)}
                      </p>
                    </td>
