...
export default function SettingsPage() {
  ...
  const { data: wallets } = useQuery({ queryKey: ['inst-wallets'], queryFn: users.wallets });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-2xl">
        ...
        >
          <div className="grid grid-cols-2 gap-3">
            <Field label="First name" error={profileForm.formState.errors.firstName?.message}>
              <input className={inputCls} {...profileForm.register('firstName')} />
            </Field>
            <Field label="Last name" error={profileForm.formState.errors.lastName?.message}>
              <input className={inputCls} {...profileForm.register('lastName')} />
            </Field>
          </div>
          <Field label="Email" hint="Email cannot be changed">
            <input className={inputCls + ' opacity-60'} value={user?.email ?? ''} disabled />
          </Field>
          <button type="submit" disabled={profileMutation.isPending} className={btnPrimary}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Identity Verification</h2>
        </div>
          ...
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Entity KYC status</p>
                {(() => {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        >
          <Field label="Current password" error={passwordForm.formState.errors.currentPassword?.message}>
            <input type="password" className={inputCls} {...passwordForm.register('currentPassword')} />
          </Field>
          <Field label="New password" hint="Minimum 8 characters" error={passwordForm.formState.errors.newPassword?.message}>
            <input type="password" className={inputCls} {...passwordForm.register('newPassword')} />
          </Field>
          <Field label="Confirm new password" error={passwordForm.formState.errors.confirm?.message}>
            <input type="password" className={inputCls} {...passwordForm.register('confirm')} />
          </Field>
          <button type="submit" disabled={passwordMutation.isPending} className={btnPrimary}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Connected Wallets</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              Up to {MAX_WALLETS} wallets · Signature required to verify ownership
            </p>
          </div>
        ...
        <div className="px-6 py-4">
          {!wallets?.length ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">
              No wallets connected. Click "Add Wallet" and sign the verification message in MetaMask.
            </p>
          ) : (
              ...
              {wallets.map((w) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5 font-mono">{w.address}</p>
                  </div>
