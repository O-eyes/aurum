...
export async function request<T>(
  ...
  if (!res.ok) {
    ...
    throw new ApiError(res.status, body, (body as any)?.message ?? `HTTP ${res.status}`);
  }
...
export interface Order {
  id: string; userId: string; type: 'BUY' | 'SELL'; status: string;
  amountUsd: string; goldOunces: string; tokenAmount: string;
  goldPriceUsd: string; walletAddress: string; idempotencyKey: string;
  createdAt: string; updatedAt: string;
}

export interface BalanceResponse { balance: string; balanceUsd: string; goldPriceUsd: string; }

export const auth = {
  login: async (body: { email: string; password: string }) => {
    const data = await request<{ accessToken: string; refreshToken: string; user: UserProfile }>(
      '/auth/login', { method: 'POST', body: JSON.stringify(body), skipAuth: true },
    );
    setTokens(data.accessToken, data.refreshToken);
    return data;
  },
...
export const orders = {
  ...
  create: (body: { type: 'BUY' | 'SELL'; amountUsd: string; walletAddress: string; idempotencyKey: string }) =>
    request<Order>('/orders', { method: 'POST', body: JSON.stringify(body) }),
};

export const kyc = {
  status: () => request<{ status: string; reviewNote: string | null; sdkToken: string | null; externalId: string | null; updatedAt: string | null }>('/kyc/status'),
  submit: (body: { idDocumentType?: string }) =>
    request<{ sdkToken: string; externalId: string }>('/kyc/submit', { method: 'POST', body: JSON.stringify(body) }),
};
