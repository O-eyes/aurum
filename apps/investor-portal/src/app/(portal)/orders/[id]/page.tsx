...
import { orders as ordersApi, payments, mint, ApiError, type RedemptionMetadata } from '@/lib/api';
...
import { ArrowLeft, Flame, ExternalLink, CheckCircle2, Circle, Package, MapPin } from 'lucide-react';
...
const PICKUP_STEPS  = ['BURN_CONFIRMED', 'PROCESSING', 'READY_FOR_PICKUP', 'COMPLETED'] as const;
const DELIVERY_STEPS = ['BURN_CONFIRMED', 'PROCESSING', 'SHIPPED', 'DELIVERED'] as const;
...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mobileProvider, setMobileProvider] = useState<'mtn' | 'telecel' | 'at'>('mtn');
  ...
  const { isLoading: isConfirming, isSuccess: txConfirmed } = useWaitForTransactionReceipt({ hash: burnTxHash });
  ...
  const burnRequestMutation = useMutation({
    ...
    onSuccess: async (txData) => {
      try {
        ...
        const rejected = e?.code === 4001 || e?.message?.toLowerCase().includes('user rejected');
        toast[rejected ? 'warning' : 'error'](
          rejected ? 'Transaction cancelled.' : 'Transaction failed. Check your wallet balance and try again.',
        );
      }
    },
    onError: (e) =>
      toast.error(e instanceof ApiError ? e.message : 'Failed to prepare burn transaction'),
  });
  ...
  const cancelMutation = useMutation({
    ...
    onError: (e) => toast.error(e instanceof ApiError ? e.message : 'Failed to cancel order'),
  });

  // Buy payment mutations
  const cardPayMutation = useMutation({
    mutationFn: () =>
      payments.initiateCard(id, { email: user?.email ?? '', callbackUrl: `${window.location.origin}/dashboard` }),
    onSuccess: (data) => { window.location.href = data.authorizationUrl; },
    onError: (e) => setPayError(e instanceof ApiError ? e.message : 'Card payment failed'),
  });

  const mobilePayMutation = useMutation({
    mutationFn: () => payments.initiateMobileMoney(id, { phone, provider: mobileProvider }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['orders', id] });
      toast.success('Mobile money prompt sent. Approve it on your phone to complete payment.');
    },
    onError: (e) => setPayError(e instanceof ApiError ? e.message : 'Mobile money payment failed'),
  });

  if (isLoading) return <div className="flex justify-center py-20"><Spinner /></div>;
  if (!order) return <div className="p-6 text-center"><p className="text-sm text-gray-500">Order not found.</p></div>;

  const statusColor = ORDER_STATUS_COLORS[order.status] ?? 'bg-gray-100 text-gray-700';
  const explorerBase = process.env.NEXT_PUBLIC_EXPLORER_URL ?? 'https://etherscan.io';
  const walletMatches = !!address && address.toLowerCase() === order.walletAddress.toLowerCase();
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        <Link href="/orders" className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Order Details</h1>
      </div>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          <CardTitle>
            {order.type === 'BUY' ? 'Buy' : order.type === 'SELL' ? 'Sell' : 'Redemption'} Order
          </CardTitle>
        ...
        <CardContent>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div>
              ...
              <dd className="font-mono text-xs text-gray-700 dark:text-gray-300 mt-1 break-all">{order.id}</dd>
            </div>
            <div>
              <dt className="text-gray-500 dark:text-gray-400">Created</dt>
              <dd className="text-gray-900 dark:text-gray-100 mt-1">{formatDate(order.createdAt)}</dd>
            </div>
            ...
            {order.type === 'REDEEM' && rm ? (
              <>
                ...
                <div>
                  <dt className="text-gray-500 dark:text-gray-400">Total weight</dt>
                ...
                <div>
                  <dt className="text-gray-500 dark:text-gray-400">AUR to burn</dt>
                  <dd className="text-gray-900 dark:text-gray-100 font-semibold mt-1">
                ...
                <div>
                  ...
                  <dd className="text-gray-900 dark:text-gray-100 mt-1 flex items-center gap-1">
                    {rm.collectionMethod === 'pickup'
                      ? <><MapPin className="h-3 w-3 text-gray-400" /> Vault pickup</>
                      : <><Package className="h-3 w-3 text-gray-400" /> Home delivery</>}
                  </dd>
                </div>
              </>
            ) : (
              <>
                <div>
                  ...
                  <dd className="text-gray-900 dark:text-gray-100 font-semibold mt-1">
                    ...
                    <p className="text-xs text-gray-400 font-normal mt-0.5">{formatUsd(order.amountUsd)}</p>
                  </dd>
                </div>
                <div>
                  <dt className="text-gray-500 dark:text-gray-400">Gold ounces</dt>
                  <dd className="text-gray-900 dark:text-gray-100 mt-1">{parseFloat(order.goldOunces).toFixed(6)} oz</dd>
                </div>
                <div>
                ...
                <div>
                  <dt className="text-gray-500 dark:text-gray-400">Gold price at order</dt>
                  <dd className="text-gray-900 dark:text-gray-100 mt-1">{formatUsd(order.goldPriceUsd)}/oz</dd>
                </div>
              </>
            )}
            ...
            {rm?.collectionMethod === 'pickup' && rm.preferredPickupDate && (
              <div className="col-span-2">
                <dt className="text-gray-500 dark:text-gray-400">Preferred pickup date</dt>
                <dd className="text-gray-900 dark:text-gray-100 mt-1">{rm.preferredPickupDate}</dd>
              </div>
            )}
            ...
            {rm?.trackingNumber && (
              <div className="col-span-2">
                ...
                <dd className="mt-1 flex items-center gap-2">
                  <span className="font-mono text-xs text-gray-700 dark:text-gray-300">{rm.trackingNumber}</span>
                  {rm.trackingUrl && (
                    <a href={rm.trackingUrl} target="_blank" rel="noopener noreferrer" className="text-gold-600 hover:text-gold-700">
      ...
      {order.type === 'BUY' && order.status === 'PAYMENT_PENDING' && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button className="w-full" onClick={() => cardPayMutation.mutate()} loading={cardPayMutation.isPending}>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs text-gray-500">
                <span className="bg-white dark:bg-gray-800 px-2">or pay via mobile money</span>
              </div>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <Select
                  ...
                  onChange={(v) => setMobileProvider(v as typeof mobileProvider)}
                  className="mt-1"
                />
              </div>
              <Input label="Phone number" type="tel" placeholder="0244000000" value={phone} onChange={(e) => setPhone(e.target.value)} />
              <Button variant="secondary" className="w-full" onClick={() => mobilePayMutation.mutate()} loading={mobilePayMutation.isPending} disabled={!phone}>
      ...
      {(order.type === 'SELL' || order.type === 'REDEEM') && BURN_NEEDED.has(order.status) && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Flame className="h-4 w-4 text-orange-500" />
              {order.type === 'REDEEM' ? 'Burn Tokens to Claim Gold' : 'Burn Tokens to Complete Sale'}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Alert variant="info">
              Burn <strong>{parseFloat(order.tokenAmount).toFixed(4)} AUR</strong> from{' '}
              ...
              {order.type === 'REDEEM'
                ...
                : <>receive your payout of {ghsRate ? formatGhs(order.amountUsd, ghsRate) : formatUsd(order.amountUsd)}{ghsRate ? ` (${formatUsd(order.amountUsd)})` : ''}.</>}
            </Alert>

            {!isConnected && (
              <Alert variant="warning">No wallet connected. Connect the wallet holding your AUR tokens to continue.</Alert>
            )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                    {isConfirming ? 'Waiting for block confirmation…'
                      : confirmBurnMutation.isPending ? 'Notifying server…'
                      : 'Burn confirmed on-chain'}
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <p className="font-mono text-xs text-gray-500 dark:text-gray-400 break-all flex-1">{burnTxHash}</p>
                  <a href={`${explorerBase}/tx/${burnTxHash}`} target="_blank" rel="noopener noreferrer" className="shrink-0 text-gold-600 hover:text-gold-700 mt-0.5">
                    <ExternalLink className="h-3.5 w-3.5" />
                  </a>
                </div>
            ...
            <p className="text-xs text-gray-400 text-center">
              Burning tokens is irreversible. Only proceed when ready to complete this{' '}
              {order.type === 'REDEEM' ? 'redemption' : 'sale'}.
            </p>
          </CardContent>
        </Card>
      )}
      ...
      {order.type === 'SELL' && BURN_PROCESSING.has(order.status) && (
        <Alert variant="info" title="Payout processing">
          ...
          <strong>{ghsRate ? formatGhs(order.amountUsd, ghsRate) : formatUsd(order.amountUsd)}</strong>
          {ghsRate ? ` (${formatUsd(order.amountUsd)})` : ''} is being processed and will arrive within 1–2 business days.
        </Alert>
      )}
      ...
      {order.type === 'REDEEM' && !BURN_NEEDED.has(order.status) && order.status !== 'CANCELLED' && (
        <Card>
          <CardHeader>
            <CardTitle>
              {rm?.collectionMethod === 'delivery' ? 'Delivery Status' : 'Collection Status'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <RedemptionTimeline
...
function RedemptionTimeline({
  ...
  const steps = collectionMethod === 'delivery' ? [...DELIVERY_STEPS] : [...PICKUP_STEPS];
  const currentIdx = steps.indexOf(currentStatus as never);

  return (
    <ol className="space-y-4">
      {steps.map((step, i) => {
        const done    = i < currentIdx || currentStatus === step && ['COMPLETED', 'DELIVERED'].includes(step);
          ...
          <li key={step} className="flex items-start gap-3">
            ...
            <div>
              <p className={cn(
                ...
                current ? 'text-gold-700 dark:text-gold-400' :
              ...
              {current && step === 'READY_FOR_PICKUP' && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Bring your ID to {CUSTODIAN.vaultAddress} ({CUSTODIAN.pickupHours})
                </p>
              )}
            </div>
            {i < steps.length - 1 && (
              <div className={cn('ml-2.5 mt-5 h-4 w-px', done ? 'bg-green-300 dark:bg-green-700' : 'bg-gray-200 dark:bg-gray-700')} />
            )}
          </li>
        );
      })}
