...
import { formatUsd, formatOz, formatGhs, formatGhsDirect, generateIdempotencyKey } from '@/lib/utils';
...
export default function BuyPage() {
  ...
  useEffect(() => {
    if (embeddedWallet.enabled && embeddedWallet.address) {
      setValue('walletAddress', embeddedWallet.address, { shouldValidate: true });
    }
  ...
  const estimatedOz = goldPriceUsd && goldCostUsd ? goldCostUsd / goldPriceUsd : null;
  ...
  const confirmAmountGhs = pendingOrder ? parseFloat(pendingOrder.amountGhs) : 0;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Buy Gold Tokens</h1>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit((data) => setPendingOrder(data))} className="space-y-4">
              <div>
                <Input
                  label="Amount (GH₵)"
                  type="number"
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {GHS_PRESETS.map((p) => (
                    <button
                      key={p}
                      type="button"
                      onClick={() => setValue('amountGhs', String(p), { shouldValidate: true })}
              ...
              {embeddedWallet.enabled ? (
                embeddedWallet.address ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-emerald-700/80 dark:text-emerald-400/80 font-mono truncate">
                          {embeddedWallet.address.slice(0, 10)}…{embeddedWallet.address.slice(-6)}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="rounded-lg border border-gray-200 dark:border-gray-700 px-3 py-3 text-center">
                    ...
                    <Button type="button" size="sm" variant="secondary" onClick={embeddedWallet.setup} disabled={!embeddedWallet.ready}>
                      Set up wallet
                    </Button>
                  </div>
                )
                ...
                <div>
                  ...
                  />
                  {address && (
                    <button type="button" className="mt-1 text-xs text-gold-600 hover:underline"
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={!kycApproved || (embeddedWallet.enabled && !embeddedWallet.address)}
              >
                Review Order
              </Button>
            </form>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="border-t border-gray-100 dark:border-gray-700 pt-3 space-y-2">
                <Row
                  ...
                  value={grossGhs ? `− ${formatGhsDirect(platformFeeGhs.toFixed(2))}` : '—'}
                />
                {TENANT.fees.taxPercent > 0 && (
                  <Row
                    label={`Tax (${TENANT.fees.taxPercent}%)`}
                    value={grossGhs ? `− ${formatGhsDirect(taxGhs.toFixed(2))}` : '—'}
                  />
                )}
                <Row
                  label="Gold cost"
                  value={grossGhs ? formatGhsDirect(goldCostGhs.toFixed(2)) : '—'}
                  sub={goldCostUsd ? `≈ ${formatUsd(goldCostUsd)}` : undefined}
                />
              </div>
              <div className="border-t border-gray-100 dark:border-gray-700 pt-3 space-y-2">
                <Row label="Gold (est.)" value={estimatedOz ? formatOz(estimatedOz) : '—'} />
                <Row label="AUR tokens" value={estimatedOz ? `${estimatedOz.toFixed(6)} AUR` : '—'} highlight bold />
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/30">
              <CardContent className="pt-4 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  {[
                    { label: 'Collateral Ratio', value: `${(parseFloat(snapshot.backingRatio) * 100).toFixed(2)}%` },
                    { label: 'Gold in Reserve', value: `${parseFloat(snapshot.goldHeldOz).toFixed(2)} oz` },
                    { label: 'Circulating', value: `${parseFloat(snapshot.tokenSupply).toFixed(2)} AUR` },
                  ].map((s) => (
                    <div key={s.label}>
                      <p className="text-xs text-emerald-600 dark:text-emerald-500">{s.label}</p>
                      <p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">{s.value}</p>
                    </div>
                  ))}
                </div>
...
function Row({ label, value, sub, bold, highlight }: { label: string; value: string; sub?: string; bold?: boolean; highlight?: boolean }) {
  return (
    <div className="flex justify-between">
      <span className="text-gray-500 dark:text-gray-400">{label}</span>
      <div className="text-right">
        <span className={`${bold ? 'font-semibold' : ''} ${highlight ? 'text-gold-600 dark:text-gold-400' : 'text-gray-900 dark:text-gray-100'}`}>
          {value}
        </span>
