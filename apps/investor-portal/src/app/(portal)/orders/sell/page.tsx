...
export default function SellPage() {
  ...
  const estimatedTokens =
    goldPriceUsd && amountUsd ? amountUsd / goldPriceUsd : null;
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      await payments.setPayoutMethod(order.id, {
        ...
        phone:
          data.payoutMethod === "mobile_money" ? data.momoPhone : undefined,
        network:
          data.payoutMethod === "mobile_money" ? data.momoProvider : undefined,
        accountNumber:
          data.payoutMethod === "bank_transfer"
            ? data.accountNumber
            : undefined,
        bankCode:
          data.payoutMethod === "bank_transfer" ? data.bankName : undefined,
      });
    ...
    onSuccess: (order) => {
      toast.success(
        "Sell order created. Proceed to burn your tokens.",
        "Order placed",
      );
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() =>
                        setValue(
                          "payoutMethod",
                          value as FormData["payoutMethod"],
                        )
                      }
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onChange={(v) =>
                          setValue(
                            "momoProvider",
                            v as FormData["momoProvider"],
                          )
                        }
                      />
                      {errors.momoProvider && (
                        <p className="mt-1 text-xs text-red-500">
                          {errors.momoProvider.message}
                        </p>
                      )}
                    </div>
                    <Input
                ...
                {payoutMethod === "bank_transfer" && (
                  <>
                    ...
                    <p className="text-xs text-gray-400">
                      Bank transfers are processed manually by our treasury team
          ...
          <div className="lg:col-span-2">
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <Row
                  label="Your balance"
                  value={`${tokenBalance.toFixed(4)} AUR`}
                />
                <Row
                  label="Gold price"
                ...
                <Row
                  ...
                  value={
                    estimatedTokens ? `${estimatedTokens.toFixed(6)} AUR` : "—"
                  }
                />
  ...
  return (
    <div className="flex justify-between">
      ...
      <div className="text-right">
        <span
          className={`${bold ? "font-semibold" : ""} text-gray-900 dark:text-gray-100`}
        >
          {value}
        </span>
        {sub && (
          <p
            className={`text-xs mt-0 ${subHighlight ? "text-green-600 dark:text-green-400 font-medium" : "text-gray-400"}`}
          >
            {sub}
          </p>
        )}
