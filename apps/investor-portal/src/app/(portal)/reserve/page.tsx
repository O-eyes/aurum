    ...
    <div className="p-4 sm:p-6 space-y-8 max-w-3xl">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Every AUR token is backed by physical gold held in custody. This page
          shows the current reserve position in real time.
        </p>
      </div>

      {/* ── Backing ratio hero ── */}
      <div
        className={`rounded-2xl p-6 ${isFullyBacked ? "bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800" : "bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800"}`}
      >
        <div className="flex items-center gap-3 mb-4">
          <ShieldCheck
            className={`h-6 w-6 ${isFullyBacked ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}
          />
          <span
            className={`text-sm font-semibold uppercase tracking-wide ${isFullyBacked ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400"}`}
          >
            {isFullyBacked ? "Fully Backed" : "Under-Collateralised"}
          </span>
        </div>
        <p
          className={`text-6xl font-bold tabular-nums ${isFullyBacked ? "text-emerald-800 dark:text-emerald-300" : "text-red-800 dark:text-red-300"}`}
        >
          {ratioPercent}%
        </p>
        <p
          className={`mt-1 text-sm ${isFullyBacked ? "text-emerald-600 dark:text-emerald-500" : "text-red-600 dark:text-red-500"}`}
        >
        ...
        <CardContent className="space-y-4 text-sm text-gray-600 dark:text-gray-400">
          <ol className="space-y-3">
            {[
              {
                n: "1",
                t: "Gold is purchased and deposited",
                d: `Physical gold is procured and deposited into ${CUSTODIAN.name}'s insured vault before any tokens are minted.`,
              },
              {
                n: "2",
                t: "Tokens are minted 1:1",
                d: "One AUR token represents one troy ounce of gold. The smart contract only mints tokens after a reserve snapshot confirms sufficient gold coverage.",
              },
              {
                n: "3",
                t: "Reserve is verified on-chain",
                d: "The collateral ratio is updated each time gold is added or tokens are burned. The data on this page is pulled directly from the latest verified snapshot.",
              },
              {
                n: "4",
                t: "Redemptions burn tokens",
                d: "When you redeem physical gold, the corresponding AUR tokens are permanently burned, removing them from circulation and reducing the token supply.",
              },
            ].map(({ n, t, d }) => (
        ...
        <CardContent>
          <dl className="grid grid-cols-1 gap-3 sm:grid-cols-2 text-sm">
            {[
              ...
              {
                label: "Insurance",
                value: CUSTODIAN.insuranceIncluded
                  ? "Included on all holdings"
                  : "Not included",
              },
              {
                label: "Delivery regions",
                value: CUSTODIAN.deliveryRegions.join(", "),
              },
            ].map(({ label, value }) => (
              <div key={label}>
