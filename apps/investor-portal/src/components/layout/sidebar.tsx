...
import {
  LayoutDashboard, ShoppingCart, History, ShieldCheck,
  LogOut, Coins, Sun, Moon, X, Settings, Gem,
} from 'lucide-react';

const ACTIVE_STATUSES = new Set([
  'PENDING', 'PAYMENT_PENDING', 'PAYMENT_CONFIRMED', 'MINTING',  // buy
  'BURN_PENDING', 'BURN_CONFIRMED', 'PAYOUT_PROCESSING',          // sell
]);
...
const KYC_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  PENDING:      { label: 'Unverified', cls: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' },
};
...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const pendingCount = orderList?.filter((o) => ACTIVE_STATUSES.has(o.status)).length ?? 0;
  ...
  const navItems = [
    ...
    { label: 'Order History', href: '/orders',         icon: History, badge: pendingCount },
      ...
      <aside className={cn(
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base font-bold text-gray-900 dark:text-white">Aurum</span>
          </div>
          <button onClick={onClose} className="md:hidden text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="border-t border-gray-100 dark:border-gray-700 p-3 space-y-1">
          ...
          <div className="px-3 py-1 space-y-1">
            ...
            <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium', kycInfo.cls)}>
              {kycInfo.label}
            </span>
          </div>
