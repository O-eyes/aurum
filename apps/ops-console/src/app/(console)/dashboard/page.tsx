...
export default function DashboardPage() {
  ...
  const pendingKyc =
    kycQueue?.filter((k) => k.status === "UNDER_REVIEW").length ?? 0;
  const activeOrders =
    allOrders?.filter((o) =>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            >
              <RefreshCw
                className={`h-3 w-3 ${snapFetching ? "animate-spin" : ""}`}
              />
              Refresh
            </button>
          </div>
          <div className="px-6 py-4">
            {snapLoading ? (
              ...
              <dl className="space-y-3 text-sm">
                <Row
                  label="Gold held"
                  value={`${parseFloat(snapshot.goldHeldOz).toFixed(6)} oz`}
                />
                <Row
                  label="Token supply"
                  value={`${parseFloat(snapshot.tokenSupply).toFixed(4)} AUR`}
                />
                <Row
                  label="Gold price"
                  value={formatUsd(snapshot.goldPriceUsd) + "/oz"}
                />
                <Row
                  label="Backing ratio"
                  value={`${(parseFloat(snapshot.backingRatio) * 100).toFixed(4)}%`}
                  highlight={
                    parseFloat(snapshot.backingRatio) < 0.98
                      ? "text-red-600"
                      : "text-green-600"
                  }
                />
                <Row
                  label="Last snapshot"
                  value={formatDate(snapshot.snapshotedAt)}
                />
              </dl>
            ) : (
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <a href="/kyc" className="text-xs text-brand-500 hover:underline">
              View all →
            </a>
          </div>
...
function StatCard({
  ...
  return (
    <div
      className={`bg-white rounded-xl border shadow-sm p-5 flex items-start gap-4 ${alert ? "border-orange-300" : "border-gray-200"}`}
    >
      <div className="rounded-lg bg-gray-50 p-2">{icon}</div>
      <div>
        <p className="text-xs text-gray-500 font-medium">{title}</p>
        <p
          className={`text-2xl font-bold ${alert ? "text-orange-600" : "text-gray-900"}`}
        >
          {value}
        </p>
