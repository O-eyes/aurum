...
export default function KycReviewPage() {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) =>
      kycApi.reject(id, {
        note: note || "Documents did not meet requirements",
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["kyc-queue"] });
      setSelected(null);
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm space-y-4 p-5">
              <div>
                <h3 className="text-sm font-semibold text-gray-900">
                  ...
                  {selected.user.lastName ?? "Applicant"}
                </h3>
              ...
              <dl className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <dd className="font-mono text-gray-700 break-all max-w-[140px]">
                ...
                {selected.externalId && (
                  <div className="flex justify-between">
                    ...
                    <dd className="font-mono text-gray-700">
                      {selected.externalId.slice(0, 12)}…
                    </dd>
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <dd>
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${KYC_STATUS_COLORS[selected.status] ?? ""}`}
                    >
                      {selected.status}
                    </span>
                  </dd>
