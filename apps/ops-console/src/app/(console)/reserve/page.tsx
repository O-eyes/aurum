...
export default function ReservePage() {
  ...
  const snapshotMutation = useMutation({
    mutationFn: () => reserve.manualSnapshot({
      goldHeldOz,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Reserve Dashboard</h1>
        <button
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Manual Reserve Snapshot</h2>
            <button onClick={() => { setShowForm(false); setFormError(''); }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="flex items-end">
              ...
              >
                {snapshotMutation.isPending && <RefreshCw className="h-3.5 w-3.5 animate-spin" />}
          ...
          <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
            This will record the current gold weight and automatically fetch the live gold price to compute the backing ratio.
          </p>
        </div>
      )}
      ...
      {!latestLoading && latest && (
        <div className={`flex items-center gap-3 rounded-xl border p-4 ${
          ...
          <div>
            <p className={`font-semibold text-sm ${healthy ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>
              {healthy ? 'Reserve fully backed' : 'Under-collateralization alert'}
            </p>
            <p className={`text-xs mt-0.5 ${healthy ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-400'}`}>
              Backing ratio: {ratio !== null ? `${(ratio * 100).toFixed(4)}%` : '—'}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Gold Held', value: latest ? `${parseFloat(latest.goldHeldOz).toFixed(6)} oz` : '—' },
          { label: 'Token Supply', value: latest ? `${parseFloat(latest.tokenSupply).toFixed(4)} AUR` : '—' },
          { label: 'Gold Price', value: latest ? formatUsd(latest.goldPriceUsd) + '/oz' : '—' },
          { label: 'Reserve Value', value: latest ? formatUsd(parseFloat(latest.goldHeldOz) * parseFloat(latest.goldPriceUsd)) : '—' },
        ].map((item) => (
          <div key={item.label} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <p className="text-lg font-bold text-gray-900 dark:text-gray-100 mt-1">{item.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Snapshot History</h2>
        </div>
        {historyLoading ? (
          ...
          <p className="py-8 text-center text-sm text-gray-400">No snapshots yet</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <th className="px-6 py-3 text-left font-medium">Gold Held (oz)</th>
                  <th className="px-6 py-3 text-left font-medium">Token Supply</th>
                  <th className="px-6 py-3 text-left font-medium">Gold Price</th>
                  <th className="px-6 py-3 text-left font-medium">Backing Ratio</th>
                </tr>
              </thead>
              <tbody>
                {history.map((snap) => {
                  ...
                  return (
                    <tr key={snap.id} className="border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      <td className="px-6 py-3 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                        {formatDate(snap.snapshotedAt)}
                      </td>
                      <td className="px-6 py-3 text-gray-900 dark:text-gray-100">{parseFloat(snap.goldHeldOz).toFixed(6)}</td>
                      <td className="px-6 py-3 text-gray-600 dark:text-gray-400">{parseFloat(snap.tokenSupply).toFixed(4)} AUR</td>
                      <td className="px-6 py-3 text-gray-500 dark:text-gray-500">{formatUsd(snap.goldPriceUsd)}</td>
                      <td className="px-6 py-3">
                        <span className={`font-semibold ${r >= 0.98 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                          {(r * 100).toFixed(4)}%
                        </span>
                      </td>
                    </tr>
