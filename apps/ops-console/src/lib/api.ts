...
export const auth = {
  login: async (body: { email: string; password: string }) => {
    ...
    if (!hasAccess) throw new ApiError(403, null, 'Insufficient permissions for ops console');
    setTokens(data.accessToken, data.refreshToken);
    return data;
  },
  logout: async () => {
...
export const kyc = {
  ...
  approve: (id: string, body: { note?: string }) =>
    request<void>(`/kyc/${id}/approve`, { method: 'POST', body: JSON.stringify({ reason: body.note }) }),
  reject: (id: string, body: { note: string }) =>
    request<void>(`/kyc/${id}/reject`, { method: 'POST', body: JSON.stringify({ reason: body.note }) }),
};
...
export const reserve = {
  ...
  manualSnapshot: (body: { goldHeldOz: string; vaultReference?: string }) =>
    request<ReserveSnapshot>('/reserve/snapshot', { method: 'POST', body: JSON.stringify(body) }),
};
