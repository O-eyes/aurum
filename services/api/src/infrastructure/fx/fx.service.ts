...
export class FxService {
  ...
  constructor(
    ...
    this.apiUrl = config.get<string>('fx.apiUrl') ?? 'https://open.er-api.com/v6/latest/USD';
  }
  ...
  async getUsdRate(currency: string): Promise<Prisma.Decimal> {
    ...
    await this.redis.set(`${CACHE_KEY_PREFIX}${upper}`, rate.toFixed(8), CACHE_TTL_SECONDS);
  ...
  async usdToSmallestUnit(amountUsd: Prisma.Decimal, currency: string, multiplier = 100): Promise<number> {
    const rate = await this.getUsdRate(currency);
    return amountUsd.mul(rate).mul(multiplier).toDecimalPlaces(0, Prisma.Decimal.ROUND_HALF_UP).toNumber();
  }
  ...
  private async fetchRate(currency: string): Promise<Prisma.Decimal> {
    ...
    if (!response.ok) {
      throw new ServiceUnavailableException(`FX rate feed returned HTTP ${response.status}`);
    }
    ...
    if (typeof rate !== 'number' || rate <= 0) {
      throw new ServiceUnavailableException(`FX rate for ${currency} unavailable`);
    }
