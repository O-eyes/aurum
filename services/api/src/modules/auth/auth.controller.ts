...
import { WalletChallengeSchema, type WalletChallengeDto } from './dto/wallet-challenge.dto';
import { WalletVerifySchema, type WalletVerifyDto } from './dto/wallet-verify.dto';
import { VerifyEmailSchema, type VerifyEmailDto } from './dto/verify-email.dto';
import { RequestOtpSchema, VerifyOtpSchema, type RequestOtpDto, type VerifyOtpDto } from './dto/otp.dto';
...
export class AuthController {
  ...
  @Post('otp/request')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Send a 6-digit verification code to a phone number' })
  async requestOtp(
  ...
  @Post('otp/verify')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Verify the code — signs in, creating the account on first use' })
  async verifyOtp(
  ...
  async logout(
    ...
    return this.authService.logout(user.id, payload.jti, user.sessionId, requestId);
  }
