...
export class AuthService {
  ...
  async verifyEmail(token: string, requestId: string) {
    ...
    const record = verification as
      | (typeof verification & { user: { id: string; email: string } })
      | null;
  ...
  async requestOtp(phone: string, requestId: string) {
    ...
    await this.sms.send(
      phone,
      `Your Aurum verification code is ${code}. Valid for 10 minutes.`,
    );
  ...
  async verifyOtp(
    ...
    const key = `otp:${phone}`;
    const stored = await this.redis.getJson<{ code: string; attempts: number }>(
      key,
    );

    if (!stored) {
      throw new UnauthorizedException(
        "Code expired or not requested. Request a new one.",
      );
    }
  ...
  async walletVerify(
    ...
    if (!stored)
      throw new BadRequestException("Challenge expired or not issued");
    ...
    if (!wallet)
      throw new NotFoundException(
        "No account linked to this wallet. Sign up first.",
      );
    ...
    if (!user.emailVerified && !user.phoneVerified) {
      throw new UnauthorizedException(
        "Account not verified. Complete registration first.",
      );
    }
