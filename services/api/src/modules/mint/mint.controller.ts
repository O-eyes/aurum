...
export class MintController {
  ...
  @ApiOperation({
    summary: "[Ops] Submit mint transaction for a confirmed order",
  })
  async requestMint(
  ...
  async confirmMint(
    @Body(ZodValidationPipe.for(ConfirmMintSchema))
    body: { mintRequestId: string },
    @Req() req: FastifyRequest,
  ) {
    const requestId = req.headers[REQUEST_ID_HEADER] as string;
  ...
  @ApiOperation({
    summary:
      "Create a burn request for a SELL order and get the on-chain call details",
  })
  async requestBurn(
  ...
  async confirmBurn(
    @Body(ZodValidationPipe.for(ConfirmBurnSchema))
    body: { burnRequestId: string; txHash: string },
    @Req() req: FastifyRequest,
  ) {
    const requestId = req.headers[REQUEST_ID_HEADER] as string;
    return this.mintService.confirmBurn(
      body.burnRequestId,
      body.txHash,
      requestId,
    );
