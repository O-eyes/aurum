...
export class PaymentsController {
  ...
  @ApiOperation({ summary: 'Initiate a card payment for an order via Paystack' })
  async initiateCard(
    @CurrentUser() user: AuthenticatedUser,
    @Param('orderId') orderId: string,
    @Body(ZodValidationPipe.for(InitiateCardPaymentSchema)) dto: InitiateCardPaymentDto,
  ...
  @ApiBearerAuth()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Initiate a mobile money payment for an order via Paystack' })
  async initiateMobileMoney(
    @CurrentUser() user: AuthenticatedUser,
    @Param('orderId') orderId: string,
    @Body(ZodValidationPipe.for(InitiateMobileMoneySchema)) dto: InitiateMobileMoneyDto,
    @Req() req: FastifyRequest,
  ) {
    const requestId = req.headers[REQUEST_ID_HEADER] as string;
    return this.paymentsService.initiateMobileMoney(orderId, user.id, dto, requestId);
  }
}
...
export class PayoutsController {
  ...
  @ApiBearerAuth()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Set payout method (bank or mobile money) for a SELL order' })
...
export class PaymentsWebhookController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Paystack unified webhook — handles charge + transfer events' })
  async paystackWebhook(
    ...
    if (event.startsWith('transfer.')) {
      return this.payoutsService.handleTransferWebhook(rawBody, signature, requestId);
    }
