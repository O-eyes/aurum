...
export class ReserveService {
  ...
  async scheduledSnapshot() {
    if (!this.rpcUrl || this.tokenAddress === '0x0000000000000000000000000000000000000000') {
    ...
    try {
      ...
      if (!goldHeldOz) {
        this.logger.warn('Skipping scheduled snapshot: vault holdings unavailable');
        return;
      }
      ...
      this.logger.error('Scheduled reserve snapshot failed', (err as Error).message);
    }
  }
  ...
  async getLatest() {
    const snap = await this.db.reserveSnapshot.findFirst({ orderBy: { createdAt: 'desc' } });
    return snap ? { ...snap, snapshotedAt: snap.createdAt.toISOString() } : null;
  }
  ...
  async getHistory(limit = 30, offset = 0) {
    ...
    return snaps.map((s) => ({ ...s, snapshotedAt: s.createdAt.toISOString() }));
  }
  ...
  private async saveSnapshot(params: {
    ...
    const { goldHeldOz, tokenSupply, goldPriceUsd, vaultReference, source } = params;
    ...
    if (backingRatio.lessThan(BACKING_RATIO_ALERT_THRESHOLD)) {
      this.logger.warn(`⚠️  Backing ratio below threshold: ${backingRatio.toFixed(4)}`);
      await this.kafka.publish(
  ...
  private async getOnChainTokenSupply(): Promise<Prisma.Decimal> {
    const client = createPublicClient({ chain: this.resolveChain(), transport: http(this.rpcUrl) });
  ...
  private async getLedgerTokenSupply(): Promise<Prisma.Decimal> {
    ...
    const raw = (result._sum.amount as Prisma.Decimal | null) ?? new Prisma.Decimal(0);
